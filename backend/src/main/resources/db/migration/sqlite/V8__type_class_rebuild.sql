-- =============================================================
-- 类型类(Type Classes)模块重构 — 升级版需求(260625)
-- 旧单表 ont_type_class(定义+绑定混合) → 拆为 4 类表:
--   1) ont_dic_type_class            统一枚举字典
--   2) ont_type_class_category_dict  一级大类(Kind)字典
--   3) ont_type_class                类型类标准定义(元数据)
--   4) ont_type_class_bind           类型类绑定实例
-- SQLite 无 JSONB → JSON 用 TEXT 存。
-- =============================================================
