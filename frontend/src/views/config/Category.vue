...
const allowedCategoryTypes = computed(() => {
  ...
  if (pt == null) return ALL_CATEGORY_TYPES.filter(t => t.value === 1)
  if (pt === 1) return ALL_CATEGORY_TYPES.filter(t => t.value >= 2)
  if (pt === 2) return ALL_CATEGORY_TYPES.filter(t => t.value === 3)
  return ALL_CATEGORY_TYPES.filter(t => t.value === 3)
})
